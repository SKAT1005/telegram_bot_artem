...
async def channels(order, wave_time):
    ...
    if start <= now:
        ...
        if (not order.next_action_time or order.next_action_time.timestamp() <= datetime.datetime.now().timestamp()) and len(no_subscribe) > 0:
            user = no_subscribe[0]
...
async def start():
    while True:
        ...
        for order in Orders.objects.all():
            if order.url[-3:] == 'bot':
                if datetime.datetime.now().timestamp() >= order.end.timestamp():
                    order.delete()
                else:
